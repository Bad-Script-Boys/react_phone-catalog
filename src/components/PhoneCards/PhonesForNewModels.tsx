...
export const PhonesForNewModels: React.FC<Props> = ({
  image,
  name,
  fullPrice,
  screen,
  ...
  return theme === 'light' ? (
    <div className="darkPhoneCard w-72 bg-white shadow-lg rounded-lg p-8 transition-transform duration-500 transform hover:scale-105">
      ...
      <div className="mt-4 w-full">
        <p className="text-xl font-bold text-black mt-2 text-left mb-3">{`$${fullPrice}`}</p>
      </div>
      <section>
    ...
    <div className="w-72 bg-[#161827] shadow-lg rounded-lg p-8 transition-transform duration-500 transform hover:scale-105">
      <Link to={`/${product.category}/${product.itemId}`}>
        <img
          src={`${image}`}
          alt="img"
      ...
      <div className="mt-4 w-full">
        <p className="text-xl font-bold text-white mt-2 text-left mb-3">{`$${fullPrice}`}</p>
      </div>
      <section>
        <div className="flex justify-between text-gray-600">
          <h3 className="text-[#75767F]">{`Screen`} </h3>
